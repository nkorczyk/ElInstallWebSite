package com.sebastiaanholtrop.reflection {
	...
	public class Reflect extends EventDispatcher {
		...
		private function spriteToFlippedBitmapData():void {
			...
			switch (this.side) {
				case Reflect.LEFT: case Reflect.RIGHT:
					flipMatrix.scale(-1, 1);
					flipMatrix.translate(this._sourceSprite.width, 0);
				break;
				case Reflect.BOTTOM: case Reflect.TOP:
					flipMatrix.scale(1, -1);
		...
		private function createGradientBitmapData():void {
			...
			switch (side) {
				case Reflect.LEFT: rotation = 1; break;
				case Reflect.RIGHT: rotation = 0; break;
				case Reflect.BOTTOM: rotation = 0.5; break 
				case Reflect.TOP: rotation = 1.5; break;
			}
		...
		private function addToSource():void {
			...
			switch (this.side) {
				case Reflect.LEFT: resultBm.x = - this._sourceSprite.width - this._distance; break;
				case Reflect.RIGHT: resultBm.x = this._sourceSprite.width + this._distance; break;
				case Reflect.BOTTOM: resultBm.y = this._sourceSprite.height + this._distance; break 
				case Reflect.TOP: resultBm.y = - this._sourceSprite.height - this._distance; break;
			}
		...
		public function set distance(distance:Number):void {
			...
			if (this.add) {
				...
				switch (this.side) {
					case Reflect.LEFT: resultBm.x = -  this._sourceSprite.width - distance; break;
					case Reflect.RIGHT: resultBm.x =  this._sourceSprite.width + distance; break;
					case Reflect.BOTTOM: resultBm.y =  this._sourceSprite.height + distance; break 
					case Reflect.TOP: resultBm.y = -  this._sourceSprite.height - distance; break;
				}
			}
		}
